...
--  Agenda
--  ---------------------------------------------------------------------------
--  Je eigen planning: opnames, deadlines, afspraken. Alleen jij ziet dit.
--  Deadlines van projecten worden apart opgehaald en hoeven hier niet in.
-- ---------------------------------------------------------------------------
...
create table if not exists public.agenda (
  ...
  herhaal_tot  date,
  aanvraag_id  uuid references public.project_requests(id) on delete set null
);

create index if not exists agenda_datum_idx on public.agenda (datum);


-- ---------------------------------------------------------------------------
--  Bibliotheek
--  ---------------------------------------------------------------------------
--  Links naar tutorials, inspiratie en tools. Alleen jij ziet dit.
-- ---------------------------------------------------------------------------

create table if not exists public.bibliotheek (
  id           uuid primary key default gen_random_uuid(),
  created_at   timestamptz not null default now(),
  titel        text not null,
  url          text not null,
  ...
  bron         text,
  video_id     text,
  laatst_open  timestamptz,
  aantal_open  integer not null default 0
);

create index if not exists bibliotheek_created_idx on public.bibliotheek (created_at desc);
create index if not exists bibliotheek_map_idx on public.bibliotheek (map);
...
alter table public.bibliotheek      enable row level security;

-- Agenda en bibliotheek zijn puur voor jou: bezoekers hebben hier geen enkele
-- toegang, ook niet om te lezen.
drop policy if exists "alleen ingelogd beheert de agenda" on public.agenda;
create policy "alleen ingelogd beheert de agenda"
  on public.agenda for all to authenticated using (true) with check (true);

drop policy if exists "alleen ingelogd beheert de bibliotheek" on public.bibliotheek;
create policy "alleen ingelogd beheert de bibliotheek"
  on public.bibliotheek for all to authenticated using (true) with check (true);
