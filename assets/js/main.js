...
function imgTag(src, alt) {
  if (!src) return "";
  /* loading="lazy" laadt de afbeelding pas wanneer ze in beeld komt, en
     decoding="async" houdt het scrollen vloeiend. Beide helpen ook mee voor
     je vindbaarheid, want Google weegt laadsnelheid mee. */
  return `<img src="${esc(src)}" alt="${esc(alt)}" hidden loading="lazy" decoding="async"
      ...
      $("#team-leden").innerHTML = leden
        ...
        <figure class="lid reveal">
          <div class="frame frame--vierkant" data-label="Foto">
            ${l.foto ? imgTag(l.foto, l.naam + (l.functie ? ", " + l.functie : "") + " bij JoppeDS Studio\u0027s") : ""}
          </div>
          <figcaption>
    ...
    .map((p, i) => {
      ...
      <article class="src">
        ...
        <div class="frame frame--16x9" data-label="${esc(p.titel)} — 16:9">
          ${imgTag(src, p.titel + " — project van JoppeDS Studio\u0027s")}
        </div>
      </article>`;
    })
    .join("");
